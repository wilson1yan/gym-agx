...
def build_simulation():
    ...
    logger.debug(f"({round(new_node_x, 4)} | {round(new_node_y, 4)}) - initial node")
    ...
    logger.debug(f"Angle towards center with respect to x-axis: {round(math.degrees(rope_angle), 2)}°")
    # rope.add(agxCable.FreeNode(0.0, 0.0, 0))
    ...
    for i in range(add_node_amount):
        ...
        logger.debug(f"Added node {i + 1} at ({round(new_node_x, 4)} | {round(new_node_y, 4)}) - rope angle: {round(math.degrees(rope_angle), 2)}° ")
        rope.add(agxCable.FreeNode(new_node_x, new_node_y, rope_z))

    print(f"Rope length: {rope.getCurrentLength()}")
    ...
    # => caused stiff straight connections between nodes and weird "curly" rope ends
    # rope.rebind()
...
def get_random_goal(args):
    # print(f"Shutdown state before AutoInit(): {agx.isShutdown()}")

    # print(f"Shutdown state after AutoInit(): {agx.isShutdown()}")
    goal_path = main(args)
    # print(f"Shutdown state after main(): {agx.isShutdown()}")
    # print(f"Shutdown state after shutdown(): {agx.isShutdown()}")
    return goal_path
