...
OBSTACLE_POSITIONS = [[0.0,-0.0125], [0.0,0.0125]]
R_OBSTACLE = 0.001
...
def build_simulation(sim):
    ...
    if not GRAVITY:
        ...
        g = agx.Vec3(0, 0, -1)  # remove gravity
    ...
    for i in range(0, len(obs_pos)):
        obstacle = create_body(name=f"obstacle_{i}", 
                               shape=agxCollide.Cylinder(2 * R_OBSTACLE, 0.05),
