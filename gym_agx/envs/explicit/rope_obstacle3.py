...
class RopeObstacle3Env(agx_env.AgxEnv):
    ...
    def __init__(self, n_substeps=1, reward_type="dense", observation_type="gt", headless=False, 
                 pushers=None, max_episode_length=100, **kwargs):
    ...
    def _build_simulation(self):
        pusher_init_pos = (np.random.uniform(-0.75, 0.75, size=2) * LENGTH).tolist()
    ...
    def reset(self):
        ...
        force0 = np.random.uniform(-0.1, 0.1, size=2).tolist()
        force1 = np.random.uniform(-0.1, 0.1, size=2).tolist()
